       ...
       )
{
	...
	num_elem_ifmap = BN_R * BN_I * (R * BS_R) * BS_I * I;
	...
	num_elem_ofmap = BN_R * BN_O * R * BS_R * O * BS_O;
...
//	CHECK(status);
	...
	CHECK(status);
//	status = clSetKernelArg(kernel, 3, sizeof(cl_mem), &d_cfg);
//	CHECK(status);
//	status = clSetKernelArg(kernel, 4, sizeof(cl_mem), &d_DEBUG_msg);
//	CHECK(status);
...
void report_measurement()
{
	...
	for (int i = 0; i < num_conv; i++) 
	{
		//num_operations += (2.0 * cfg_in_num[i] * cfg_out_num[i] * cfg_img_row[i]);
		num_operations += (2.0 * BN_R * BS_R * R * BN_O * BS_O * O * BN_I * BS_I * I);
	}
